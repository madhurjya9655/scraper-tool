...
# -*- coding: utf-8 -*-
"""
b2b_lead_scraper_async.py

Async, directory-safe B2B industrial scraper for India.
- Free-tier SERP APIs (SerpAPI + Serper; optional)
- Extreme dedupe (domain/name/email/phone/title/linkedin)
- Shallow, fast site crawl with strict filters
- Output schema (strict 13 columns):
...
import os, re, csv, json, sys, time, math, logging, asyncio, urllib.parse, sqlite3, random
...
UA = os.environ.get("SCRAPER_USER_AGENT", "Mozilla/5.0 (compatible; B2BLeadScraperAsync/1.3)")
ALANG = os.environ.get("SCRAPER_ACCEPT_LANGUAGE", "en-IN,en;q=0.9")

TARGET_LOCATIONS = [
    "Aurangabad","Chennai","Coimbatore","Gurgaon","Indore","Jamshedpur","Pune","Rajkot",
    "Faridabad","Ankleshwar","Anand","Vitthal Udyognagar","Vadodara","Vapi","Daman",
    "Perambur","Sriperumbudur","Pimpri","Chinchwad","Chakan","Mumbai","Bhiwandi","Satara","Shirwal"
]
...
DIRECTORY_DOMAINS: Set[str] = {
    ...
    "indianindustriesdirectory.com","sulekha.com","enggpro.com","vapiindustries.org",
    # NEW marketplaces/people/portals to block
...
SERP_NEVER: Set[str] = DIRECTORY_DOMAINS.union({
    ...
    "bseindia.com","nseindia.com","moneycontrol.com","gem.gov.in","bidplus.gem.gov.in",
    "gov.in","nic.in","cdind.org","dcmsme.gov.in"
})
BAD_HOST_SUFFIXES = (".gov.in",".nic.in",".ac.in",".edu",".edu.in")

TITLE_BLACKLIST = {
    "home","jobs","account suspended","login","sign in","register","instagram",
    "share market","stock market","technology & data for venture capital","bse","nse",
    # catch marketplace/catalog style
    "trader","b2b marketplace","marketplace","catalog","directory","price","prices"
}

PATH_BLACKLIST_SUBSTR = (
    "/login","/signin","/register","/account","/careers","/jobs",
    "/blog/","/news","/events","/investor","/stock","/gallery","/privacy","/terms"
)

# Lightweight classifier signals
MUST_HAVE_KEYWORDS = (
    "manufactur","forg","machin","cnc","hydraul","gear","flange","crankshaft","valve","pump","auto component"
)
...
CANDIDATE_PATHS = ["/","/contact","/contact-us","/about","/about-us","/team","/people","/reach-us"]
...
def is_dir_domain(host: Optional[str]) -> bool:
    ...
    if h.endswith(BAD_HOST_SUFFIXES): return True
    # generic heuristics
    bad_tokens = ("directory","yellowpages","association","chamber","stock","exchange","marketplace")
...
def industry_for_kw(kw: str) -> str:
    for i in TARGET_INDUSTRIES:
        if i.split()[0].lower() in kw.lower(): return i
    return "Industrial"

# ---- NEW: generic-title detection ----
GENERIC_TITLE_PREFIXES = ("find ", "buy ", "best ", "top ", "price", "prices")
def title_looks_generic(t: str) -> bool:
    t = (t or "").strip().lower()
    if any(t.startswith(p) for p in GENERIC_TITLE_PREFIXES):
        return True
    # one-word or purely generic nouns
    words = re.findall(r"[a-zA-Z]+", t)
    if len(words) <= 1:
        return True
    generic_nouns = {"home","jobs","trader","products","services","contact","about","catalog","marketplace"}
    if len(words) <= 3 and all(w in generic_nouns for w in words):
        return True
    return False

# ---- NEW: location validation ----
def location_ok(city: str, text: str) -> bool:
    txt = (text or "").lower()
    c = city.lower()
    aliases = {
        "pimpri": {"pimpri","pimpri-chinchwad","pcmc"},
        "chinchwad": {"chinchwad","pimpri-chinchwad","pcmc"},
        "gurgaon": {"gurgaon","gurugram"},
        "mumbai": {"mumbai","bombay"},
        "vadodara": {"vadodara","baroda"}
    }
    pool = {c} | aliases.get(c, set())
    return any(x in txt for x in pool)

def looks_like_company_site(url: str, title: str, page_snippet: str) -> bool:
    """Cheap gate to reject portals/social/gov/stock and generic pages."""
...
class LeadsDB:
    ...
    def _schema(self):
        ...
        self.conn.commit()
    def upsert(self, row: Dict[str, Any]) -> Optional[int]:
        cols = ["company_name","contact_person","email","phone","website","industry","location",
                "company_type","source","scraped_date","linkedin_url","enriched_emails"]
        vals = [row.get(k,"") for k in cols]
        try:
            cur = self.conn.cursor()
            cur.execute(f"""
                INSERT OR IGNORE INTO leads ({",".join(cols)})
                VALUES ({",".join(["?"]*len(cols))});
            """, vals)
            self.conn.commit()
            return cur.lastrowid if cur.rowcount>0 else None
        except Exception as e:
            self.logger.warning(f"DB upsert failed: {e}")
            return None
    ...
    async def post_json(self, url: str, payload: Dict[str,Any], headers: Optional[Dict[str,str]]=None, retries: int=3) -> Optional[bytes]:
        ...
        for attempt in range(1, retries+1):
            try:
                ...
                if attempt<retries:
                    await asyncio.sleep(1.0 + random.uniform(0,1))
                    continue
    ...
    async def search(http: Http, q: str, location: str, num: int=10) -> List[str]:
        ...
        try:
            ...
            for it in js.get("organic_results",[]) or []:
                link = it.get("link")
                if not link or not is_valid_url(link): continue
                h = domain_of(link) or ""
...
QUERY_TEMPLATES = [
    '{kw} manufacturers in {city} India -indiamart -tradeindia -justdial -yellowpages -exportersindia -sulekha -enggpro -site:instagram.com -site:facebook.com',
    'site:.in {kw} {city} supplier -indiamart -tradeindia -justdial -facebook -youtube -pinterest -sulekha -enggpro',
    '{kw} {city} factory address -indiamart -tradeindia -justdial -exportersindia -bseindia -nseindia -gov.in',
    '{kw} near {city} company website -indiamart -tradeindia -justdial -sulekha',
    '{kw} {city} oem tier 1 -indiamart -tradeindia -justdial -gov.in',
    'inurl:about OR inurl:contact "{kw}" "{city}" -indiamart -tradeindia -justdial -exportersindia -sulekha -enggpro',
    'site:.in "{kw}" "{city}" -indiamart -tradeindia -justdial -gov.in -bseindia.com -nseindia.com'
]
...
def extract_emails(text: str) -> List[str]:
    found = [m.group(0).strip() for m in EMAIL_RE.finditer(text or "")]
...
def should_scan_path(path: str) -> bool:
    return any(path.rstrip("/").lower().endswith(p.strip("/")) for p in CANDIDATE_PATHS)
...
class SiteScanner:
    ...
    async def crawl(self, site_url: str) -> Tuple[List[str], List[str], Optional[str], str, str]:
        """
        Returns: (emails, phones, linkedin_url, page_title, last_page_text_snippet)
        """
        ...
        for p in CANDIDATE_PATHS:
            ...
            last_text = text[:2000]  # small snippet for crude scoring
            ...
            # Early break if solid contact found
...
class Scraper:
    def __init__(self, base_dir: str, args: Dict[str, Any]):
        ...
        self.sources = args.get("sources", ["serpapi","serper"])
    ...
    async def serp_seed(self, http: Http, kw: str, city: str) -> List[str]:
        ...
        for qi in rnd:
            ...
            if "serpapi" in self.sources:
                r1 = await SerpAPI.search(http, q, city, num=self.limit_per_combo*2)
                batch.extend(r1)
            if "serper" in self.sources:
                r2 = await Serper.search(http, q, city, num=self.limit_per_combo*2)
                batch.extend(r2)

            # de-dup same host and blocklist
            uniq: Dict[str,str] = {}
            for u in batch:
            ...
            # accumulate and early stop if repetition rises
            new_domains = [h for h in uniq.keys() if not self.dedupe.seen_domain(uniq[h])]
            urls.extend(list(uniq.values()))
            if len(new_domains) <= max(2, math.ceil(0.3*len(uniq))):
                break
            if len(urls) >= self.limit_per_combo*3 or self.time_up():
                break

        # return one URL per domain (homepage canonicalized)
        outs=[]
        for u in urls:
            p = urllib.parse.urlparse(u)
    ...
    async def process_site(self, scanner: SiteScanner, site_url: str, kw: str, city: str) -> Optional[Dict[str, Any]]:
        ...
        # Reject noisy sites early
        if not looks_like_company_site(site_url, title, snippet):
            return None

        # NEW: location gate (title or snippet must show the target city)
        if not location_ok(city, f"{title} {snippet}"):
            return None
        ...
        if self.dedupe.is_duplicate(company, site_url, phone or None, email or None, li, title):
            return None
        ...
        row = {
            ...
            "source": site_url,  # SERP/Website
            "scraped_date": iso_now(),
            "linkedin_url": li or "",
            "enriched_emails": ""
        }

        # Final zero-tolerance host gate
        row_dom = domain_of(row["website"])
        if is_dir_domain(row_dom):
            return None

        # Minimum output quality (hard gate)
        has_corp_email = row["email"] and row_dom and (row["email"].split("@")[-1].lower() == row_dom)
        has_phone = bool(row["phone"])
        title_l = (title or "").lower()
        has_mfg_title = any(k in title_l for k in MUST_HAVE_KEYWORDS)

        # If no corp email, require BOTH phone AND strong manufacturing signal in title
        if not (has_corp_email or (has_phone and has_mfg_title)):
            return None

        if row["email"] and any(row["email"].lower().endswith("@"+d) for d in DIRECTORY_DOMAINS):
            return None
    ...
    async def run_combo(self, http: Http, kw: str, city: str, scanner: SiteScanner):
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if rid:
                ...
                self.logger.info(f"Skipped (duplicate): {row['company_name']}")
...
def parse_cli():
    import argparse
    p = argparse.ArgumentParser(description="Async B2B industrial lead scraper (directory-safe)")
